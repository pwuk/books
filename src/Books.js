...
const Books = () => {
  ...
  const [bookMetaData, loading] = useFetchBooks(page, size, filter, {
    count: 0,
    books: [],
  });
  ...
  const pageSelectionHandler = (page) =>
    history.push(`/${page}/${size}/${filter}`);
  const pageSizeSelectionHandler = (size) =>
    history.push(`/${1}/${size}/${filter}`);
  const setFilter = () => {
  ...
  return (
    <div className={"App-header"}>
      <div className={"page-component"}>
        ...
        <button onClick={clearFilter} disabled={!searchInput.current.value}>
          Clear
        </button>
        {loading ? (
          <div>Loading...</div>
          ...
          <div>
            {bookMetaData.count === 0
              ? "No matching books"
              : `Found ${bookMetaData.count} book(s)`}
          </div>
        )}
      </div>
      <div className={"page-component"}>
      ...
      {bookMetaData.count > 0 && (
        <>
          <PageSizeSelector
            currentSelection={parseInt(size, 10)}
            pageSizeSelectionHandler={pageSizeSelectionHandler}
            totalRecords={bookMetaData.count}
          />
